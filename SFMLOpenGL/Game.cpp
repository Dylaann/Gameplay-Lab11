...
Game::Game() : 
	renderWindow(VideoMode(800, 600), 
...
void Game::run()
{
	...
	View mainView(sf::FloatRect(0, 0, 400, 600));
	...
	while (isRunning){
		...
		while (renderWindow.pollEvent(event))
		{
			...
			{
				// Set Model Rotation
				model = rotate(model, turnSpeed, glm::vec3(0, 1, 0)); // Rotate
			}
			...
			{
				// Set Model Rotation
				model = rotate(model, -turnSpeed, glm::vec3(1, 0, 0)); // Rotate
			}
			...
			{
				// Set Model Rotation
				model = translate(model, glm::vec3(-0.05, 0, 0)); // left
			}

			else if (sf::Keyboard::isKeyPressed(sf::Keyboard::D))
			{
				// Set Model Rotation
				model = translate(model, glm::vec3(0.05, 0, 0)); //right
			}
			else if (sf::Keyboard::isKeyPressed(sf::Keyboard::W))
			{
				// Set Model Rotation
				model = translate(model, glm::vec3(0, 0.05, 0)); // Up
			}

			else if (sf::Keyboard::isKeyPressed(sf::Keyboard::S))
			{
				// Set Model Rotation
				model = translate(model, glm::vec3(0, -0.05, 0)); //Down
			}
		}
		update();
		render();
